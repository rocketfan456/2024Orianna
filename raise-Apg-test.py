...
class TestTankset(unittest.TestCase):
    def test_al2219_oxygen_single_tank(self):
        ...
        np.testing.assert_approx_equal(tanks.thkDomesCalc, 0.0008902876,
                                       5)  # required dome thickness (before minimum comparison)
        np.testing.assert_approx_equal(tanks.thkCylCalc, 0.001780575,
                                       5)  # required cylinder thickness (before minimum comparison)
    ...
    def test_al2219_oxygen_two_tanks(self):
        ...
        np.testing.assert_approx_equal(tanks.thkDomesCalc, 0.000689613,
                                       5)  # required dome thickness (before minimum comparison)
        np.testing.assert_approx_equal(tanks.thkCylCalc, 0.001379227,
                                       5)  # required cylinder thickness (before minimum comparison)
        np.testing.assert_approx_equal(tanks.thkDomes, 0.004, 5)  # thickness of dome used (after minimum comparison)
        np.testing.assert_approx_equal(tanks.thkCyl, 0.004, 5)  # thickness of cylinder used (after minimum comparison)
    ...
    def test_stainless_hydrogen_one_tank(self):
        ...
        np.testing.assert_approx_equal(tanks.thkDomesCalc, 0.002142338,
                                       5)  # required dome thickness (before minimum comparison)
        np.testing.assert_approx_equal(tanks.thkCylCalc, 0.00428467,
                                       5)  # required cylinder thickness (before minimum comparison)
        np.testing.assert_approx_equal(tanks.thkDomes, 0.00214233,
                                       5)  # thickness of dome used (after minimum comparison)
        np.testing.assert_approx_equal(tanks.thkCyl, 0.004284676,
                                       5)  # thickness of cylinder used (after minimum comparison)
...
class TestSubsystems(unittest.TestCase):
    def test_oxygen_hydrogen_large(self):
        ...
        np.testing.assert_approx_equal(subs.nrgTotalMargin, 19314.2857142,
                                       5)  # total energy of battery including depth of discharge
        ...
        np.testing.assert_approx_equal(subs.mDryWithoutStructure, 2465.609886482621,
                                       5)  # mass of everything but structure and gear
    ...
    def test_oxygen_methane_small(self):
        ...
        np.testing.assert_approx_equal(subs.nrgTotalMargin, 5942.85714285,
                                       5)  # total energy of battery including depth of discharge
        ...
        np.testing.assert_approx_equal(subs.mDryWithoutStructure, 846.95856175,
                                       5)  # mass of everything but structure and gear
...
class TestMission(unittest.TestCase):
    def test_sequence(self):
        ...
        PreTLISett = cf.Phase('Pre-TCM1 Settling', 10000, 0, engMain, 'Settling', 0, mdotRCS, mdotOxBoiloff,
                              mdotFuelBoiloff)
        ...
        CoastToTCM2 = cf.Phase('Coast to TCM2', TLI.mEnd, 0, engMain, 'Coast', 2 * 86400, mdotRCS, mdotOxBoiloff,
                               mdotFuelBoiloff)
        ...
        np.testing.assert_approx_equal(Mission.mPropImpulseFuel, 164.4979685628946,
                                       5)  # Total power draw including margin
        np.testing.assert_approx_equal(Mission.mPropImpulseMono, 20.54824802773328, 5)  # solar array mass
        np.testing.assert_approx_equal(Mission.mPropImpulseReserve, 21.795700873730965,
                                       5)  # total energy for battery without depth of discharge
        np.testing.assert_approx_equal(Mission.mPropImpulseReserveOx, 18.094776541918407,
                                       5)  # total energy of battery including depth of discharge
        np.testing.assert_approx_equal(Mission.mPropImpulseReserveFuel, 3.2899593712578925, 5)  # mass of battery
        np.testing.assert_approx_equal(Mission.mPropImpulseReserveMono, 0.4109649605546656,
                                       5)  # mass of electrical subsystem
        ...
        np.testing.assert_approx_equal(Mission.mPropConsumedOx, 919.7554819097854,
                                       5)  # mass of everything but structure and gear
        np.testing.assert_approx_equal(Mission.mPropConsumedFuel, 189.53127819062448,
                                       5)  # mass of everything but structure and gear
        ...
        np.testing.assert_approx_equal(Mission.mPropAtLandingMono, 0.7306570193207347,
                                       5)  # Total power draw including margin
        np.testing.assert_approx_equal(Mission.mPropAtLandingTotal, 33.422107892632894, 5)  # solar array mass
        np.testing.assert_approx_equal(Mission.mPropTotalOx, 947.2287610362209,
                                       5)  # total energy for battery without depth of discharge
        np.testing.assert_approx_equal(Mission.mPropTotalFuel, 194.7494499375012,
                                       5)  # total energy of battery including depth of discharge
