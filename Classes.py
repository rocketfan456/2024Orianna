...
def PrintData(phaseList):
    ...
    for curPhase in phaseList:
        print('{0:20s}{1:11.1f}{2:11.1f}{3:11.1f}'.format(curPhase.strName, curPhase.dvPhase, curPhase.mStart,
                                                          curPhase.mEnd))


class Phase:

    def __init__(self, strName, mStart, dvPhase, clsEng, strPhaseType, dtPhase, mdotRCS, mdotOxBoiloff,
                 mdotFuelBoiloff):
...
class Subsystems:
    def __init__(self, mVehicleStart, clsEng, clsOxTankSet, clsFuelTankSet, clsMonoTankSet, pwrDrawPayload,
                 strArrayType, strLanderSize, tBattery):
        pctMarginArray = 0.3
        ...
        mStructureAndGear = mDryWithoutStructure / (1 - (pctStructure + pctLandingGear)) * (
                    pctStructure + pctLandingGear)
